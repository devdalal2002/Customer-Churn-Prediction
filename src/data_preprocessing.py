...
def clean_total_charges(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This function is permissive: if the dataframe does not contain expected
    Telco columns, it attempts a best-effort fill (NaNs -> 0) and sets the
    imputed flag where appropriate instead of raising a KeyError.
    """
    df = df.copy()
    if 'TotalCharges' not in df.columns:
        # Nothing to do for non-Telco datasets
        return df
    ...
    # If tenure and MonthlyCharges exist, apply the more sophisticated imputation
    ...
    # ensure numeric dtype and fill any remaining NaNs safely
...
def encode_categoricals(df: pd.DataFrame, drop_first: bool = True) -> pd.DataFrame:
    ...
    # exclude customerID and target-like columns (e.g., 'churn') from encoding
    cats = [c for c in df.select_dtypes(include=['object', 'category']).columns if c.lower() not in ('customerid','churn')]
    if not cats:
        return df
...
def generic_impute(df: pd.DataFrame, cfg: dict = None) -> pd.DataFrame:
    ...
    # Special-case Telco TotalCharges behaviour if configured and column exists
    if col_imps.get('TotalCharges') == 'telco_total_charges' and 'TotalCharges' in df.columns:
...
def preprocess_dataframe(df: pd.DataFrame, cfg: dict = None) -> pd.DataFrame:
    """Apply generic preprocessing steps to a dataframe and return the processed df.

    Steps:
    - generic_impute (config-driven)
    - create_features (if the function from feature_engineering is available)
    - encode categoricals (to keep a canonical processed form)

    This helper is intended for use in interactive flows (dashboard uploads or testing).
    """
    ...
    if cfg and isinstance(cfg, dict):
        ...
        df = generic_impute(df, cfg={})

    try:
        df = create_features(df)
    except Exception:
        # best-effort: if feature creation fails (missing expected cols), continue
        pass

    # Ensure a target column exists for downstream code (scoring-only or explicit training will still require labels)
    target_col = (cfg or {}).get('target_column', 'Churn')
    if target_col not in df.columns:
        # add an empty target column (NaN) so flows that expect the column do not error
        df[target_col] = pd.NA
